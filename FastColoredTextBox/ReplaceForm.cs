...
namespace FastColoredTextBoxNS
{
    public partial class ReplaceForm : Form
    {
        ...
        private void btFindNext_Click(object sender, EventArgs e)
        {
            ...
            {
                if (!Find(tbFind.Text))
                    MessageBox.Show(Replace_Form["Not found"]);
            }
            catch (Exception ex)
            {
        ...
        {
            string tmp;
            if (Replace_Form.TryGetValue(s, out tmp))
                return tmp;
            return s;
        }

        private Dictionary<string, string> Replace_Form;
        public void InitLanguage()
        {
            ...
            string path;
            string[] line;
            Replace_Form = new Dictionary<string, string>();
            if (Language == "English") return;
            //**** Replace Form ****
            path = "Language\\" + Language + "\\ReplaceForm.lang";
            line = File.ReadAllLines(path);
            foreach (string curent in line)
            {
                if (curent == "") continue;
                string[] s = curent.Split('=');
                Replace_Form.Add(s[0], s[1]);
            ...
            this.btReplace.Text = Translate("Replace");
            this.btReplaceAll.Text = Translate("Replace all");
            this.label2.Text = Translate("Replace:");
            this.Text = Translate("Find and replace");
