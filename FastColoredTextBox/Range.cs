...
namespace FastColoredTextBoxNS
{
    ...
    public class Range : IEnumerable<Place>
    {
        ...
        /// Set style for range
        /// </summary>
        public void SetStyle(Style style, HighlightType type)
        {
            //search code for style
            int code = tb.GetOrSetStyleLayerIndex(style);
            //set code to chars
            SetStyle(ToStyleIndex(code), type);
            //
            tb.Invalidate();
        }
        ...
        /// </summary>
        public void SetStyle(Style style, string regexPattern, HighlightType type)
        {
            //search code for style
            
            StyleIndex layer = ToStyleIndex(tb.GetOrSetStyleLayerIndex(style));
            SetStyle(layer, regexPattern, RegexOptions.None, type);
        }
        
        /// <summary>
        /// Set style for given regex
        /// </summary>
        public void SetStyle(Style style, Regex regex, HighlightType type)
        {
            //search code for style
            //if (IsInCommentLine(regex)) return;
            StyleIndex layer = ToStyleIndex(tb.GetOrSetStyleLayerIndex(style));
            SetStyle(layer, regex, type);
            //MessageBox.Show("");
        }
...
/*
        ...
        */
        ...
        /// Set style for given regex pattern
        /// </summary>
        public void SetStyle(StyleIndex styleLayer, string regexPattern, RegexOptions options, HighlightType type)
        {
            ...
            foreach (var range in GetRanges(regexPattern, options))
            {
                ...
                range.SetStyle(styleLayer, type);
                //
            }
            tb.Invalidate();
        }
        ...
        /// Set style for given regex pattern
        /// </summary>
        public void SetStyle(StyleIndex styleLayer, Regex regex, HighlightType type)
        {
            ...
            //range.SetStyle(styleLayer);
            {
                ...
                switch (range.Text)
                {
                    case "(*":
                        //MessageBox.Show(range.Text);
                        DoubleOpen.Add(range);
                        break;
                    case "*)":
                        DoubleClose.Add(range);
                        break;
                    case "{":
                        CurlyOpen.Add(range);
                        break;
                    case "}":
                        CurlyClose.Add(range);
                        InstClose.Add(range);
                        break;
                    case "{$":
                        InstOpen.Add(range);
                    ...
                    default:
                        ...
                        listHighlight.Add(range, styleLayer);
                        listHighlightType.Add(range, type);
                        break;
                }
        ...
        /// </summary>
        public void SetStyle(StyleIndex styleIndex, HighlightType type)
        {
            //set code to chars
            //MessageBox.Show(this.Text+"\nLocation: "+this.start.iLine.ToString()+";"+this.start.iChar+"-"+this.end.iLine+";"+this.end.iChar);            
            int fromLine = Math.Min(End.iLine, Start.iLine);
            int toLine = Math.Max(End.iLine, Start.iLine);
            int fromChar = FromX;
            int toChar = ToX;
            ...
            for (int y = fromLine; y <= toLine; y++)
            {
                ...
                {
                    Char c = tb[y][x];
                    //if (c.style != StyleIndex.None) return;
                    if (c.type != HighlightType.Defaul) return;
                    /*if(type==HighlightType.InstructionCompile)
                    {
                        if (c.type == HighlightType.String) return;
                    }
                    else 
                    if (c.type != HighlightType.Defaul) return;*/
                    c.style |= styleIndex;
        ...
        #region GiaosudautoProcessing....
        Dictionary<Range, StyleIndex> listHighlight = new Dictionary<Range, StyleIndex>();
        Dictionary<Range, HighlightType> listHighlightType = new Dictionary<Range, HighlightType>();
        List<Range> DoubleOpen = new List<Range>();
        List<Range> DoubleClose = new List<Range>();
        List<Range> CurlyOpen = new List<Range>();
        List<Range> CurlyClose = new List<Range>();
        List<Range> InstOpen = new List<Range>();
        List<Range> InstClose = new List<Range>();
        Style Curly;
        Style Double;
        Style Instruction;
        ...
        public void SetBrackets(Style Curly, Style Double, Style Instruction)
        {
            this.Curly = Curly;
            this.Double = Double;
        ...
        private class CompareRange: IComparer<Range>
        {
            public int Compare(Range left, Range right)
            {
                ...
                else if (left != null && right != null)
                {
                    ...
                    {
                        if(left.end.iLine==right.end.iLine)
                        {
                            returnValue = right.end.iChar.CompareTo(left.end.iChar);
                        }
                        else
                        {
                            returnValue = right.end.iLine.CompareTo(left.end.iLine);
                        }
                    }
                    else if (left.start.iLine==right.start.iLine)
                    {
                        returnValue = left.start.iChar.CompareTo(right.start.iChar);
                    }
                    else
                    {
                        returnValue=left.start.iLine.CompareTo(right.start.iLine);
                    }
        ...
        private void SetCurly()
        {
            foreach (Range open in CurlyOpen)
            {
                
                bool timduoc = false;
                foreach(Range close in CurlyClose)
                {
                    if(open.start<close.start)
                    {
                        Range tmp = new Range(tb, open.start, close.end);
                        listHighlight.Add(tmp, ToStyleIndex(tb.GetOrSetStyleLayerIndex(Curly)));
                        listHighlightType.Add(tmp, HighlightType.CommentCurly);
                        timduoc = true;
                        break;
                    }
                    //CurlyClose.RemoveAt(0);
                }
                if (timduoc) continue;
                else
                {
                    Range tmp = new Range(tb, open.start, tb.Range.end);
                    listHighlight.Add(tmp, ToStyleIndex(tb.GetOrSetStyleLayerIndex(Curly)));
                    listHighlightType.Add(tmp, HighlightType.CommentCurly);
                    continue;
                }
        ...
        private void SetDouble()
        {
            ...
            foreach (Range open in DoubleOpen)
            {
                bool timduoc = false;
                foreach (Range close in DoubleClose)
                {
                    if (open.start < close.start)
                    {
                        Range tmp = new Range(tb, open.start, close.end);
                        listHighlight.Add(tmp, ToStyleIndex(tb.GetOrSetStyleLayerIndex(Double)));
                        listHighlightType.Add(tmp, HighlightType.CommentDouble);
                        timduoc = true;
                        //MessageBox.Show(close.start.iLine.ToString());
                        break;
                    }
                    //DoubleClose.RemoveAt(0);
                }
                if (timduoc) continue;
                else
                {
        ...
        {
            foreach (Range open in InstOpen)
            {
                bool timduoc = false;
                foreach (Range close in InstClose)
                {
                    if (open.start < close.start)
                    {
                        Range tmp = new Range(tb, open.start, close.end);
                        listHighlight.Add(tmp, ToStyleIndex(tb.GetOrSetStyleLayerIndex(Instruction)));
                        listHighlightType.Add(tmp, HighlightType.InstructionCompile);
                        timduoc = true;
                        //MessageBox.Show(close.start.iLine.ToString());
                        break;
                    }
        ...
        public void ShowTest(List<Range> range)
        {
        ...
        public void DoHighLight()
        {
            ...
            List<Range> list= listHighlight.Keys.ToList();
            list.Sort(new CompareRange());
            foreach(var element in list)
            {
                element.SetStyle(listHighlight[element],listHighlightType[element]);
        ...
        public void SetFoldingMarkers(string startFoldingPattern, string finishFoldingPattern, HighlightType st)
        {
            SetFoldingMarkers(startFoldingPattern, finishFoldingPattern, SyntaxHighlighter.RegexCompiledOption, st);
        }
        ...
        /// <param name="startFoldingPattern">Pattern for start folding line</param>
        /// <param name="finishFoldingPattern">Pattern for finish folding line</param>
        public void SetFoldingMarkers(string startFoldingPattern, string finishFoldingPattern, RegexOptions options, HighlightType st)
        {
           // StyleIndex layer = ToStyleIndex(tb.GetOrSetStyleLayerIndex(st));
        ...
        public void ClearStyle(StyleIndex styleIndex)
        {
            ...
            for (int y = fromLine; y <= toLine; y++)
            {
                ...
                for (int x = fromX; x <= toX; x++)
                {
                    Char c = tb[y][x];
                    c.style &= ~styleIndex;
                    c.type = HighlightType.Defaul;
                    //MessageBox.Show(c.style.ToString());
                    tb[y][x] = c;
                }
            }
            //
        ...
        public bool ReadOnly 
        {
            ...
            set 
            {
                ...
                //set/clear style
                if (value)
                    SetStyle(readonlyStyle, HighlightType.Defaul);
                else
